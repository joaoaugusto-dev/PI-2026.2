...
Esta branch (`fix/db-01-integracao-dados`) fecha os três pontos.

## O que foi alterado

### 1. Correção do seed (`api/db/seed.sql`)

Reescrito para bater com o schema vigente de `0001_init.sql`:

- `categorias` → `grupos_ferramentas` (+ nova etapa populando
  `subgrupos_ferramentas`, nível que não existia no seed antigo).
- `colaboradores` sem os campos `codigo_cracha` e `cargo` (removidos do
  schema em 02/09).
- `ferramentas` usando `grupo_id`/`subgrupo_id`/`localizacao` no lugar de
  `categoria_id`/`localizacao_padrao`, e sem informar `codigo_identificacao`
  manualmente — esse valor é gerado pela trigger
  `fn_gera_codigo_identificacao` no INSERT.
- Os `INSERT`s de `colaboradores` e `ferramentas` passaram a resolver as
  chaves estrangeiras por nome (via `JOIN`) em vez de números de ID fixos,
  para não quebrar de novo se a ordem de inserção mudar.

### 2. Integração inicial com fonte externa: BrasilAPI de feriados
...
### 3. Consultas básicas sobre tabela de negócio: CRUD mínimo de ferramentas
