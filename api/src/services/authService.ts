...
export class AuthService {
  /**
   * Realiza login de usuário almoxarife/admin
   */
  ...
  static async criarSessaoConsulta(identificador: string): Promise<ConsultaSessaoResult> {
    ...
    const result = await query(
      `SELECT id, nome, matricula, codigo_cracha, setor_id, ativo 
       FROM colaboradores 
       WHERE (matricula = $1 OR codigo_cracha = $1 OR nome ILIKE $2) AND ativo = true
       LIMIT 1`,
      [termo, `%${termo}%`]
    );
    ...
    if (!colaborador) {
      throw new NotFoundError(
        'Colaborador não encontrado ou inativo para iniciar sessão de consulta',
