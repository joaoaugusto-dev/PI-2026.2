...
function criarReq(usuario?: { id: number; nome: string; papel: 'manutencao' | 'consulta' }) {
  return { usuario } as unknown as Request;
}

describe('authorize', () => {
  it('chama next() sem erro quando o papel do usuário está na lista permitida', () => {
    const req = criarReq({ id: 1, nome: 'Henrique', papel: 'manutencao' });
    const next = vi.fn();

    authorize('manutencao')(req, {} as Response, next);
